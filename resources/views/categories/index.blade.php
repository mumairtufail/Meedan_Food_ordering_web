                ...
                @foreach($categories as $category)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <form action="{{ route('categories.toggle-status', $category) }}" method="POST" class="inline-block" x-data="{ loading: false }" @submit="loading = true">
                                @csrf
                                @method('PATCH')
                                <button type="submit" :disabled="loading" class="relative min-w-[60px] text-center px-2.5 py-0.5 inline-flex items-center justify-center text-xs leading-4 font-semibold rounded-full {{ $category->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }} hover:opacity-80 cursor-pointer transition-opacity disabled:opacity-50">
                                    <span :class="{ 'invisible': loading }">{{ $category->is_active ? 'Active' : 'Inactive' }}</span>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="{{ route('categories.edit', $category) }}" class="text-primary hover:text-primary/70 mr-4">Edit</a>
                            
                            <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirm-category-deletion-{{ $category->id }}')" class="text-red-600 hover:text-red-900">
                                Delete
                            </button>

                            <x-modal name="confirm-category-deletion-{{ $category->id }}" :show="false" focusable>
                                <form method="post" action="{{ route('categories.destroy', $category) }}" class="p-6 text-left">
                                    @csrf
                                    @method('delete')
